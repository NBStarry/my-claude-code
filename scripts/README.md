...
- 比 qq-bridge.sh 更简单：无需 websocat、FIFO、keeper 进程、watchdog
...
**架构简化（相比 qq-bridge）：**
- 使用 HTTP 长轮询替代 WebSocket，无需 `websocat` 依赖
- 无 FIFO 管道，无 keeper 进程，无 watchdog，代码量减少约 30%
- 连接检查通过 `getMe` API 验证 Bot 可用性，无需 `lsof` 端口检测
